...
bot.command('instruction', async (ctx) => {
    ctx.replyWithHTML(`
Приветствуем вас в боте RatingRanger! 

Этот бот поможет автоматизировать работу с отзывами пользователей о вашем товаре или услуге.

⚡️Инструкция по работе с ботом, которая поможет вам получить максимум пользы от его использования. 

1. Добавьте описание товара или услуги. Чем точнее будет описание, тем более развернутый ответ на отзыв вы получите. 
2. Добавьте текст отзыва, на который необходимо получить ответ. 

Получите развёрнутый ответ от нашего бота
`)
});
